...
| **Email logging to HubSpot**     | Real-time, on every send            | Every transactional email the app sends (order confirmation, invoice, shipping notice, refund confirmation, quote proposal, payment instructions, tax exemption decision) is blind-copied to the portal logging address, so it appears on the recipient's Contact activity feed, their primary Company, and that Company's five most recent open deals. Runs in production only. Two exceptions: the account invitation email is deliberately excluded because its body contains a live account-setup link, and Cognito verification / password-reset messages are sent by AWS and never pass through this path. |
