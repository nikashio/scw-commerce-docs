...
* Payment Method Type (Credit Card, Credit Terms — recorded as `purchase_order`, Check, ACH/Wire)
* PO Number (for Credit Terms payments)
...
| **Credit Terms (NET30)**   | Approved B2B customers only | Auto-invoiced at checkout; customer pays the invoice within 30 days | Never |
...
| **Organization (Company)**                                  | The holder of the three business entitlements: purchase order terms with a shared credit pool, tax exemption, and Trusted Installer partner status. Managed at **Admin → Entitlements → Organizations**. See [Entitlement Request Workflows](entitlement-request-workflows.md).                                                                                                                                                                                                                                                                                                                                                                                                          |
| **Company Membership**                                      | How a person receives a company's entitlements. Two paths: their own email domain is registered to the company, or an administrator approves a guest link filed from the HubSpot contact card (**Link Guest Email to Company**, approved under **Requests → Membership Requests**). Both grant all three entitlements. A plain HubSpot association grants **nothing**; removing that association revokes an approved guest link within seconds and writes an audit entry on the company page.                                                                                                                                                                                             |
| **Credit Terms**                                            | The ability for a customer to buy now and pay later (NET30). Approved in SCW Commerce, never in HubSpot. Terms are normally held by a **company** with one shared credit pool, and a person reaches them by being a member of that company; grants made to an individual account before the company model still work. See [Credit Terms Management](credit-terms.md).                                                                                                                                                                                                                                                                                                                    |
| **Tax Exemption**                                           | Suppresses sales tax at checkout in the applicable states. Normally held by a **company** and reaching a person through membership; grants made to an individual account before the company model still work. Customers submit exemption requests with supporting documents via `POST /api/account/tax-exemption`; admins then approve or reject via the SCW admin UI (`POST /api/admin/tax-exemption-requests/[id]/approve`). Exemption type (`wholesale`, `government`, `other`, `non_exempt`), exempt regions (US state codes), and provenance (validated-by, validated-at, document reference) are stored in SCW Commerce and pushed to TaxJar. Every change is recorded in the append-only `tax_exemption_events` audit table. There is no `/api/webhooks/tax-exemption` endpoint.                       |
...
| **PO Number**                                               | Purchase Order number — the customer's internal reference for the order, required for Credit Terms (NET30) payments                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
